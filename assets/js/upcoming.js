...
let currentSort = "release_date.asc";
let currentPage = 1;
let totalPages = 1;
let currentQuery = "";
...
function createUpcomingCard(movie) {
    ...
    card.innerHTML = `
        ...
        <div class="upcoming-card-content">
            ...
            <button class="upcoming-card-button" data-movie-id="${movie.id}" data-backdrop="${backdropUrl}" data-title="${title}" data-description="${movie.overview || ''}" data-year="${year}" data-rating="${rating}">
    ...
    card.querySelector(".upcoming-card-button").addEventListener("click", (e) => {
        ...
        const runtime = movieDetails[movieId]?.runtime || "N/A";
...
async function fetchUpcoming(page = 1) {
    ...
    if (currentQuery) {
        ...
        endpoint = `/movie/upcoming?sort_by=${currentSort}&page=${page}`;
    }
    ...
    if (data.results) {
        const results = data.results
            ...
            .sort((a, b) => {
                if (currentSort.includes("release_date.asc")) {
                    return new Date(a.release_date) - new Date(b.release_date);
                } else if (currentSort.includes("release_date.desc")) {
                    return new Date(b.release_date) - new Date(a.release_date);
                } else if (currentSort.includes("popularity")) {
        ...
        if (page === 1) {
            ...
            upcomingMovies = upcomingMovies.concat(results);
        }
    }
}

function displayUpcoming(startIdx = 0, endIdx = LOAD_PER_PAGE) {
    if (startIdx === 0) {
        upcomingGrid.innerHTML = "";
    }
...
loadMoreBtn.addEventListener("click", async () => {
    if (displayedCount >= upcomingMovies.length && currentPage < totalPages) {
        currentPage += 1;
        await fetchUpcoming(currentPage);
    }
    displayUpcoming(displayedCount, displayedCount + LOAD_PER_PAGE);
});
